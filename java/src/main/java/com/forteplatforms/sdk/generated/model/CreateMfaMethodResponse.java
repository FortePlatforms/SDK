/*
 * OpenAPI definition
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package com.forteplatforms.sdk.generated.model;

import java.net.URLEncoder;
import java.nio.charset.StandardCharsets;
import java.util.StringJoiner;
import java.util.Objects;
import java.util.Map;
import java.util.HashMap;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;


import com.forteplatforms.sdk.generated.invoker.ApiClient;
/**
 * CreateMfaMethodResponse
 */
@JsonPropertyOrder({
  CreateMfaMethodResponse.JSON_PROPERTY_MFA_METHOD_ID,
  CreateMfaMethodResponse.JSON_PROPERTY_SECRET,
  CreateMfaMethodResponse.JSON_PROPERTY_OTPAUTH_URI,
  CreateMfaMethodResponse.JSON_PROPERTY_WEB_AUTHN_CREATION_OPTIONS
})
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", comments = "Generator version: 7.22.0")
public class CreateMfaMethodResponse {
  public static final String JSON_PROPERTY_MFA_METHOD_ID = "mfaMethodId";
  @javax.annotation.Nonnull
  private String mfaMethodId;

  public static final String JSON_PROPERTY_SECRET = "secret";
  @javax.annotation.Nullable
  private String secret;

  public static final String JSON_PROPERTY_OTPAUTH_URI = "otpauthUri";
  @javax.annotation.Nullable
  private String otpauthUri;

  public static final String JSON_PROPERTY_WEB_AUTHN_CREATION_OPTIONS = "webAuthnCreationOptions";
  @javax.annotation.Nullable
  private String webAuthnCreationOptions;

  public CreateMfaMethodResponse() { 
  }

  public CreateMfaMethodResponse mfaMethodId(@javax.annotation.Nonnull String mfaMethodId) {
    this.mfaMethodId = mfaMethodId;
    return this;
  }

  /**
   * Get mfaMethodId
   * @return mfaMethodId
   */
  @javax.annotation.Nonnull
  @JsonProperty(value = JSON_PROPERTY_MFA_METHOD_ID, required = true)
  @JsonInclude(value = JsonInclude.Include.ALWAYS)
  public String getMfaMethodId() {
    return mfaMethodId;
  }


  @JsonProperty(value = JSON_PROPERTY_MFA_METHOD_ID, required = true)
  @JsonInclude(value = JsonInclude.Include.ALWAYS)
  public void setMfaMethodId(@javax.annotation.Nonnull String mfaMethodId) {
    this.mfaMethodId = mfaMethodId;
  }


  public CreateMfaMethodResponse secret(@javax.annotation.Nullable String secret) {
    this.secret = secret;
    return this;
  }

  /**
   * Get secret
   * @return secret
   */
  @javax.annotation.Nullable
  @JsonProperty(value = JSON_PROPERTY_SECRET, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public String getSecret() {
    return secret;
  }


  @JsonProperty(value = JSON_PROPERTY_SECRET, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setSecret(@javax.annotation.Nullable String secret) {
    this.secret = secret;
  }


  public CreateMfaMethodResponse otpauthUri(@javax.annotation.Nullable String otpauthUri) {
    this.otpauthUri = otpauthUri;
    return this;
  }

  /**
   * Get otpauthUri
   * @return otpauthUri
   */
  @javax.annotation.Nullable
  @JsonProperty(value = JSON_PROPERTY_OTPAUTH_URI, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public String getOtpauthUri() {
    return otpauthUri;
  }


  @JsonProperty(value = JSON_PROPERTY_OTPAUTH_URI, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setOtpauthUri(@javax.annotation.Nullable String otpauthUri) {
    this.otpauthUri = otpauthUri;
  }


  public CreateMfaMethodResponse webAuthnCreationOptions(@javax.annotation.Nullable String webAuthnCreationOptions) {
    this.webAuthnCreationOptions = webAuthnCreationOptions;
    return this;
  }

  /**
   * Get webAuthnCreationOptions
   * @return webAuthnCreationOptions
   */
  @javax.annotation.Nullable
  @JsonProperty(value = JSON_PROPERTY_WEB_AUTHN_CREATION_OPTIONS, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public String getWebAuthnCreationOptions() {
    return webAuthnCreationOptions;
  }


  @JsonProperty(value = JSON_PROPERTY_WEB_AUTHN_CREATION_OPTIONS, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setWebAuthnCreationOptions(@javax.annotation.Nullable String webAuthnCreationOptions) {
    this.webAuthnCreationOptions = webAuthnCreationOptions;
  }


  /**
   * Return true if this CreateMfaMethodResponse object is equal to o.
   */
  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    CreateMfaMethodResponse createMfaMethodResponse = (CreateMfaMethodResponse) o;
    return Objects.equals(this.mfaMethodId, createMfaMethodResponse.mfaMethodId) &&
        Objects.equals(this.secret, createMfaMethodResponse.secret) &&
        Objects.equals(this.otpauthUri, createMfaMethodResponse.otpauthUri) &&
        Objects.equals(this.webAuthnCreationOptions, createMfaMethodResponse.webAuthnCreationOptions);
  }

  @Override
  public int hashCode() {
    return Objects.hash(mfaMethodId, secret, otpauthUri, webAuthnCreationOptions);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class CreateMfaMethodResponse {\n");
    sb.append("    mfaMethodId: ").append(toIndentedString(mfaMethodId)).append("\n");
    sb.append("    secret: ").append(toIndentedString(secret)).append("\n");
    sb.append("    otpauthUri: ").append(toIndentedString(otpauthUri)).append("\n");
    sb.append("    webAuthnCreationOptions: ").append(toIndentedString(webAuthnCreationOptions)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    return o == null ? "null" : o.toString().replace("\n", "\n    ");
  }

  /**
   * Convert the instance into URL query string.
   *
   * @return URL query string
   */
  public String toUrlQueryString() {
    return toUrlQueryString(null);
  }

  /**
   * Convert the instance into URL query string.
   *
   * @param prefix prefix of the query string
   * @return URL query string
   */
  public String toUrlQueryString(String prefix) {
    String suffix = "";
    String containerSuffix = "";
    String containerPrefix = "";
    if (prefix == null) {
      // style=form, explode=true, e.g. /pet?name=cat&type=manx
      prefix = "";
    } else {
      // deepObject style e.g. /pet?id[name]=cat&id[type]=manx
      prefix = prefix + "[";
      suffix = "]";
      containerSuffix = "]";
      containerPrefix = "[";
    }

    StringJoiner joiner = new StringJoiner("&");

    // add `mfaMethodId` to the URL query string
    if (getMfaMethodId() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%smfaMethodId%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getMfaMethodId()))));
    }

    // add `secret` to the URL query string
    if (getSecret() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%ssecret%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getSecret()))));
    }

    // add `otpauthUri` to the URL query string
    if (getOtpauthUri() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%sotpauthUri%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getOtpauthUri()))));
    }

    // add `webAuthnCreationOptions` to the URL query string
    if (getWebAuthnCreationOptions() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%swebAuthnCreationOptions%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getWebAuthnCreationOptions()))));
    }

    return joiner.toString();
  }
}

